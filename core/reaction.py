...
async def _get_message_id(event: AstrMessageEvent) -> int | None:
    ...
    try:
        ...
        logger.debug(f"[DailySharing][reaction] 获取消息ID失败: {exc}")
    return None
...
async def set_emoji(
    ...
    try:
        ...
        logger.debug(
            f"[DailySharing][reaction] marked 已标记的消息ID={message_id}, emoji_id={emoji_id}"
        )
        return True
    except Exception as exc:
        logger.debug(f"[DailySharing][reaction] 标记失败: {exc}")
        return False
