...
class TaskDeliveryMixin:
    ...
    async def _send_image_chain(self, uid: str, img_path: str, event: AstrMessageEvent = None):
    ...
    async def _send_image_chain_with_retry(self, uid: str, img_path: str, event: AstrMessageEvent = None):
        try:
            await self._send_image_chain(uid, img_path, event)
            return
            ...
            logger.warning(f"[DailySharing] weixin_oc 图片发送失败，改用更小副本重试: {first_error}")
            await self._send_image_chain(uid, retry_path, event)

    async def send(self, uid, text, img_path, audio_path=None, video_url=None, event: AstrMessageEvent = None, image_optional: bool = False) -> bool:
        """分享内容（支持分开分享，支持语音和视频）"""
        ...
        try:
            ...
            if video_url:
                ...
                if img_not_sent_yet:
                    try:
                        await self._send_image_chain_with_retry(uid, img_path, event)
                    except Exception as image_error:
                        if image_optional and text_sent:
                            logger.warning(f"[DailySharing] 图片发送失败，已保留文字内容: {image_error}")
                            return True
                        raise
