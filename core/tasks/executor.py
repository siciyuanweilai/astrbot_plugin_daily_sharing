...
class TaskExecutorMixin:
    ...
    async def _sync_qzone_result_to_event(self, event: AstrMessageEvent, text: str, img_path: str = None):
        ...
        if target_uid and self.ctx_service._is_weixin_platform(target_uid):
            ...
            if not ok:
                logger.error("[DailySharing] 同步发送内容到会话失败")
            return
    ...
    async def async_daily_share_task(
        ...
        try:
            ...
            sent = await self.send(target_umo, content, send_img_path, audio_path, video_url, event=event)
            if not sent:
    ...
    async def execute_qzone_share(self, force_type: SharingType = None, news_source: str = None, event: AstrMessageEvent = None) -> bool:
        """完全独立的 QQ 空间执行主流程"""
        ...
        try:
            ...
            if enable_img_qzone and enable_img_global:
                if stype.value in qzone_img_allowed_types:
                    ...
                    try:
                        new_img_path = await self.image_service.generate_image(clean_qzone_content, stype, life_ctx, target_umo="qzone_broadcast")
                        if new_img_path:
