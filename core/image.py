...
class ImageService:
    ...
    async def _agent_extract_visuals(self, content: str, life_context: str) -> Dict[str, str]:
        ...
        # 2. 穿搭提示
        outfit_hint = "当前是休息时间，忽略白天外出服装，仅提取上衣、下装、袜子、睡衣或家居服。" if is_night else "当前是活动时间，提取完整的外出日常穿搭。"
...
【提取要求】
1. **主体 (subject)**：【最重要】画面的核心物体描述（例如：精致的荷花酥，层层酥皮；或者：一本封皮复古的书）。如果是纯风景或画人，此项填“无”。
