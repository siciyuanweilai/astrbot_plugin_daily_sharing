...
class ContentService:
    ...
    async def _update_history(self, key_type: str, content_summary: str):
        """更新历史记录，防止重复"""
        try:
            state = await self._load_state_safe()
            
            # 获取特定类型的历史列表 (knowledge_history 或 rec_history)
            hist_key = f"{key_type}_history"
            history = state.get(hist_key, [])
            
            # 添加新记录（只保留前20个字作为特征，避免token过长）
            ...
            await self._save_state_safe({hist_key: history})
        except Exception as e:
            logger.warning(f"[内容服务] 更新历史记录失败: {e}")

    async def _get_history_str(self, key_type: str) -> str:
        """获取历史记录字符串用于 Prompt"""
        state = await self._load_state_safe()
        history = state.get(f"{key_type}_history", [])
        if not history:
            return "无"
        return "、".join(history)
    ...
    async def _gen_greeting(self, period: TimePeriod, ctx: dict):
        ...
        address_rule = ""
        if is_group:
            address_rule = "面向群友，自然使用'大家'、'各位'或不加称呼。"
        else:
...
要求：
1. 以你的人设性格说话，真实自然
2. 绝对不要强行关联历史记录，不要因为没人说话就感到尴尬。
3. 基于当前真实时间问候
4. 忽略群聊历史，直接开启新问候
5. {'简短（50-80字）' if is_group else '可适当长一些（80-120字）'}
6. 直接输出内容，不要解释  
...
请生成{p_label}问候："""
    ...
    async def _gen_mood(self, period, ctx):
        ...
        vibe_check = """
【严重警告 - 拒绝尴尬开头】
- 严禁使用：“看你们聊得这么热火朝天”、“看大家都在潜水”、“既然群里这么安静”等评价群氛围的话。
- 请完全忽略群聊的上下文，直接分享你自己的事情。就像你突然想发一条朋友圈一样。
""" if is_group else ""
...
- 早晨：困倦、不想起、急需咖啡、早高峰的匆忙。
- 上午/下午：摸鱼的心、想喝奶茶、工作/学习的枯燥、甚至一点点崩溃。
- 傍晚/下班：解脱感、饥饿感、想躺平、路上的晚霞。
- 深夜：网抑云、想吃夜宵的纠结、甚至一点点孤独感。

请将你的人设状态与这种大众情绪结合。不要只说"我在做什么"，要说"这件事带给我的感觉"，让群友看到后会心一笑或想说"我也是"。
"""
        else:
...
4. 可适当用emoji（1-2个）
5. 基于当前真实时间感悟
6. 字数：{'80-120字' if is_group else '120-150字'}
7. 直接输出内容
你的随想："""
...
直接输出："""
    ...
    async def _gen_knowledge(self, ctx: dict):
        ...
        # 获取历史记录
        history_str = await self._get_history_str("knowledge")
...
2. **核心标准是“有趣”或“有用”**：
   - 如果是生活类，优先选**实用性强**的。
   - 如果是科普类，优先选**反直觉**或**颠覆认知**的。
   - 不要刻意追求“生僻难懂”，大众感兴趣的话题也可以。
3. 只输出关键词，不要任何解释，不要标点符号。
"""
        ...
        if is_group:
             ...
             context_instruction = """
- **关联逻辑（重要）**：
  1. **关于天气：请忽略天气信息**，除非这个知识点和天气直接相关（例如下雨讲雨水形成）。否则不要强行说“今天天气不错，分享个冷知识”，非常生硬。
...
"""
        ...
        if res:
            try:
                ...
                if matches:
                    ...
                    await self._update_history("knowledge", keyword)
                elif target_keyword:
                    await self._update_history("knowledge", target_keyword)
                else:
                    await self._update_history("knowledge", res[:10])
            except: pass
    ...
    async def _gen_rec(self, ctx: dict):
        ...
        # 获取历史记录
        history_str = await self._get_history_str("rec")
...
要求：
1. 请优先选择【口碑极佳】的作品。
2. 拒绝那些**被推荐烂了的“教科书式标准答案”**。
3. 可以是经典名作，但最好能让人有“眼前一亮”或“值得重温”的感觉。
4. 只输出作品名，不要书名号，不要解释，不要标点。
"""
        ...
        if is_group:
             ...
             context_instruction = """
- **场景筛选（重要）**：
  1. **关于天气**：只有当天气能完美烘托作品氛围时（例如：下雨天推爵士乐/致郁片；大晴天推公路片）才提。**否则请完全忽略天气**。
...
"""
        ...
        if res:
            try:
                ...
                if matches:
                    keyword = max(matches, key=len)
                    await self._update_history("rec", keyword)
                elif target_work:
                     await self._update_history("rec", target_work)
                else:
                    await self._update_history("rec", res[:10])
            except: pass
