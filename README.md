...
**重构 选题与情感逻辑：**
- 新增 选题 Agent ：不再从死板的列表里随机选词，而是让 LLM 结合历史记录“头脑风暴”出新颖的冷知识或推荐作品。
- 新增 情感分析 Agent ：TTS 语音的情感（happy/sad/angry）不再依赖简单的关键词匹配，而是由 LLM 阅读理解全文后判定。
- 新增 “共鸣策略” ：心情文案生成引入深度心理学提示词（如：忙碌时寻找“缝隙中的安宁”、休闲时传递“允许暂停的松弛感”）。
...
- 变更后：LLM 工具调用 -> 立即返回空字符串 -> 后台异步执行所有生成与发送逻辑。
